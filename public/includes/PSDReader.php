...
class PSDReader {
	...
	function getImage() {
		if (!in_array($this->infoArray['compressionType'], array(0, 1), true)) {
			$this->infoArray['error'] = 'ZIP-compressed PSD image data is not supported';
			return false;
		}
		...
		switch($this->infoArray['compressionType']) {
			...
			case 1:
				...
				$countBytes = ($this->infoArray['version id'] === 2) ? 4 : 2;
				for ($i=0; $i<($this->infoArray['rows']*$this->infoArray['channels']); $i++) $this->infoArray['scanLinesByteCounts'][] = $this->_getInteger($countBytes);
